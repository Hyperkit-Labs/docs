...
import { getGitHubEditUrl, getRelatedLinks, formatLastUpdated, getLastUpdatedDate } from '@/v01/lib/github-utils';
...
export const DocsTOC: React.FC<DocsTOCProps> = ({ items, footerHeight = 0 }) => {
  const _v01RawPathToc = usePathname();
  const pathname =
    _v01RawPathToc != null && (_v01RawPathToc === '/v0.1.0' || _v01RawPathToc.startsWith('/v0.1.0/'))
      ? _v01RawPathToc === '/v0.1.0'
        ? '/'
        : _v01RawPathToc.slice('/v0.1.0'.length) || '/'
      : _v01RawPathToc ?? '';
 ...
 const relatedLinks = relatedLinksRaw.map((l) => ({
    ...l,
    href: l.href.startsWith('http') ? l.href : (l.href === '/' ? '/v0.1.0' : '/v0.1.0' + (l.href.startsWith('/') ? l.href : '/' + l.href)),
  }));
